...
class across_models(object):
    ...
    def regression_data(self,data_dic,scenarios,models,gw_index,var,levs='no'):
        ...
        for i in range(len(models)):
            ...
            if var == 'tos':
                h = hist.tos
                h.attrs = hist.tos.attrs
                rcp = rcp585.tos
                rcp.attrs = rcp585.tos.attrs
            elif var == 'ua':
                h = hist.ua
    ...
    def regression_data_div(self,data_dic_ua,data_dic_va,scenarios,models,gw_index,var,levs='si'):
        ...
        for i in range(len(models)):
            print(models[i])
            hist_ua = data_dic_ua[scenarios[0]][models[i]]['1940-1969'][0].rename({'lat':'latitude'}).rename({'lon':'longitude'})
            rcp585_ua = data_dic_ua[scenarios[1]][models[i]]['2070-2099'][0].rename({'lat':'latitude'}).rename({'lon':'longitude'})
            hist_va = data_dic_va[scenarios[0]][models[i]]['1940-1969'][0].rename({'lat':'latitude'}).rename({'lon':'longitude'})
            rcp585_va = data_dic_va[scenarios[1]][models[i]]['2070-2099'][0].rename({'lat':'latitude'}).rename({'lon':'longitude'})
            h_ua = hist_ua.ua
            ...
            h1DJF_ua = DJF_h_ua.sel(latitude=slice(90,-90))
            if levs == 'no':
                a = 0
            else:
            ...
            h1DJF_va = DJF_h_va.sel(latitude=slice(90,-90))
            if levs == 'no':
                a = 0
            else:
                h1DJF_va = h1DJF_va.isel(plev=0)
            h1DJF_va.attrs = h_va.attrs
            h1DJF_div = mpy.divergence(h1DJF_ua,h1DJF_va)
            ...
            r1DJF_ua = DJF_r_ua.sel(latitude=slice(90,-90))
            if levs == 'no':
                a = 0
            else:
