...
def plot_ellipse(x,y,xerr,yerr,title='October - November',corr='no',x_label='Eastern Pacific Warming [K K$^{-1}$]',y_label='Central Pacific Warming [K K$^{-1}$]'):
    ...
    if corr == 'yes':
        ...
        ax.plot(story_x3, story_y_red3, 'ro',alpha = 0.6,markersize=10,label='High asym Pacific Warming')
        ax.plot(story_x4, story_y_red4, 'bo',alpha = 0.6,markersize=10,label='Low asym Pacific Warming')  
    elif corr == 'ma':
        r = np.corrcoef(x,y)[0,1]; chi = (1.26**2)*2
        ts1 = np.sqrt(((1-r**2)/(2*(1-r)))*chi)
        ts2 = np.sqrt(((1-r**2)/(2*(1+r)))*chi)
        ...
        #ax.plot(story_x1, story_y_red1, 'ro',alpha = 0.6,markersize=10,label='storylines')
        #ax.plot(story_x2, story_y_red2, 'ro',alpha = 0.6,markersize=10)
        ax.plot(story_x1, story_y_red1, 'bo',alpha = 0.6,markersize=10,label='Low asym Pacific Warming')
        ax.plot(story_x2, story_y_red2, 'ro',alpha = 0.6,markersize=10,label='High asym Pacific Warming') 
    elif corr == 'pacific':
        r = np.corrcoef(x,y)[0,1]; chi = (1.26**2)*2
        ts1 = np.sqrt(((1-r**2)/(2*(1-r)))*chi)
        ts2 = np.sqrt(((1-r**2)/(2*(1+r)))*chi)
